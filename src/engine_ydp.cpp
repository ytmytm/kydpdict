...
static QString output_tip[] = OUTPUT_TIP;	// guess what happens with 'Ò' when this is not private...
    if (index <= GetTipNumber(0))
...
QString EngineYDP::GetInfoPage(void) {
    int i;
    QString tmp;
    QString output = "<h2>SkrÛty wystÍpuj±ce w t≥umaczeniach</h2><h3>CzÍ∂Ê I - GRAMATYKA</h3>";

    for (i=0; i<GetTipNumber(1); i++) {
	tmp.setNum(i);
	output += "<a name=\""+ tmp + "\"></a><h4><font color=\"red\">"+ GetInputTip(i) + "</font></h4>" + GetOutputTip(i) + "<hr>";
    }
    output += "<h3>CzÍ∂Ê II - DZIEDZINY</h3>";
