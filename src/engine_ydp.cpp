...
char ConvertYDP::toLower(const char c) {
    const static char upper_cp[] = "A•BC∆DE FGHIJKL£MN—O”PQRSåTUVWXYZèØƒ÷‹";
    const static char lower_cp[] = "aπbcÊdeÍfghijkl≥mnÒoÛpqrsútuvwxyzüø‰ˆ¸";
...
int ConvertYDP::localeCompare(const char *w1, const char *w2) {
	...
	if (len1 != len2) {
		...
		//there are problems with german words like 'zahlen' and 'z‰hlen' - 
		//the word with umlaut is smaller
		for (i=0; i<len1; ++i) {
			if (lw1[i] != lw2[i]) {
				if (lw1[i] == '‰' || lw1[i] == 'ˆ' || lw1[i] == '¸')
					return -1;
				else if (lw2[i] == '‰' || lw2[i] == 'ˆ' || lw2[i] == '¸')
					return 1;
			}
		}
	}
